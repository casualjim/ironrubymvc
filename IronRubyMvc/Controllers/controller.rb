  ...
  module Controllers
    ...
    module Filters
      ...
      module ClassMethods
        ...
        def filter(name, options=nil)
          @action_filters ||= {}
          klass = get_options(name) || options
          unless klass.nil?
            name = :controller if name.nil? or name.is_a?(Class)
            @action_filters[name.to_sym] = ((@action_filters[name.to_sym]||[]) + [klass]) || []
          end
        end
        ...
        def action_filters
          filters = {}
          superclass.action_filters.each { |k, v| filters[k] = (filters[k]||[]) + v } if superclass.respond_to? :action_filters
          (@action_filters||{}).each { |k, v| filters[k] = (filters[k]||[]) + v }
          filters
        end
        ...
        def get_options(name, options=nil)
          return name.new if name.is_a? Class
          return options.new if options.is_a? Class
          return Object.const_get(options.to_s.split('_').map {|word| word = word.capitalize }.join('')) if options.is_a?(Symbol) or options.is_a?(String)
          return options
        end
    ...
    module Selectors
      ...
      module ClassMethods
        ...
        def name_selectors
          selectors = {}
          superclass.name_selectors.each { |k, v| selectors[k] = (selectors[k]||[]) + v } if superclass.respond_to? :name_selectors
          (@name_selectors||{}).each { |k, v| selectors[k] = (selectors[k]||[]) + v }
          selectors          
        end
				
        def method_selectors
          selectors = {}
          superclass.method_selectors.each { |k, v| selectors[k] = (selectors[k]||[]) + v } if superclass.respond_to? :method_selectors
          (@method_selectors||{}).each { |k, v| selectors[k] = (selectors[k]||[]) + v }
          selectors          
        end
